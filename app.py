...
def startup_checks(app):
    with app.app_context():
        try:
            ...
            app.logger.critical(f"Database startup check failed: {str(e)}")
            raise
        try:
            redis_client.ping()
            app.logger.info("Redis connection OK")
        except Exception as e:
            app.logger.warning(f"Redis unavailable: {str(e)}")
...
def create_app():
    ...
    # التعديل هنا: قبول الدومين الخاص بك أو أي دومين في حالة عدم التعريف
    allowed_origins = [
        "https://mohamed-khalid-portfolio.vercel.app",
        "http://localhost:5173"
    ]
    ...
    def health():
        try:
            ...
            try:
                ...
                pass
            return jsonify({"success": True, "status": "healthy", "database": True, "redis": redis_ok})
        except Exception:
